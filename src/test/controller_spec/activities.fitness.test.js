...
//TODO: Move to test-helpers? Or create separate file for extra db insertions?
async function addNewAthlete() {
  // Password encrypted with bcrypt beforehand to ease the insertion to db.
  ...
  await UserInformation.query().insert({
    userId: id,
    ...newAthleteInformation.userInformation,
  });
  ...
  await UserTeamActivities.query().insert({
    userTeamId: id,
    teamActivityId: 2,
  });
...
describe("::: ACTIVITIES CONTROLLER TESTS - Fitness/Exercises :::", () => {
  ...
  before(async () => {
    await insertData();
    await addNewAthlete(Users, UserInformation, UserTeams, UserTeamActivities);
    // Assignment separate from declaration, brackets needed. - MDN
    ({
      coachUserToken,
      trainerUserToken,
      athleteUserToken: athleteUserToken1,
    } = await logInUsers(chai));

    // Newly created athlete
    athleteUserToken2 = (
     ...
     * Business logic: Update of done exercises should happen for athlete when activity has started, not before!
     **/
    //TODO: Move to separate file, wrap inside a function the following.
  ...
  describe(":: With invalid / missing token / no priviledges ::", () => {
    it("Should return status 400 with invalid / missing token", async () => {
      let res1 = await chai.requester.get(
        "/activities/team/1/activity/fitness"
      );
  ...
  describe(":: With valid access token & priviledge ::", () => {
    ...
    describe(":: GET /activities/team/:teamId/activity/fitness ::", () => {
      ...
      it("Should return all assigned fitness activities of team", async () => {
        let res = await chai.requester
          .get("/activities/team/1/activity/fitness")
        ...
        for (let i = 0; i < resAthlete1FitnessActivities; i++) {
          ...
          resAthlete1FitnessActivities[i].exercises.forEach(
            (resExercise, ii) => {
              let athlete1Exercise = athlete1FitnessActivities[i].exercises[ii];
        ...
        for (let i = 0; i < resFitnessActivities2; i++) {
          ...
          resFitnessActivities2[i].exercises.forEach((resExercise, ii) => {
            let athlete2Exercise = athlete2FitnessActivities[i].exercises[ii];
            expect(resExercise.exercisesEquipmentId).to.eql(
    ...
    describe(":: GET /activities/team/:teamId/activity/fitness/:activityId ::", () => {
      it("Should return status 404 when unknown teamId or activityId", async () => {
        let res1 = await chai.requester
      ...
      it("Should return participant(s) with assigned exercises of fitness activity", async () => {
        let res = await chai.requester
        ...
        expect(
          res3.body.data[0].fitnessActivities[0].userTeamActivitiesId
        ).to.eql(3);
        /**
    ...
    describe(":: GET /activities/team/:teamId/activity/fitness/member/:userTeamId", () => {
      it("Should return status 404 when teamId or userTeamId unknown", async () => {
        let res1 = await chai.requester
      ...
      it("Should return team member assigned fitness activities with related exercises", async () => {
        let res = await chai.requester
          .get("/activities/team/1/activity/fitness/member/8")
          .set("Authorization", `Bearer ${athleteUserToken2}`);
        ...
        for (let i = 0; i < resFitnessActivities; i++) {
          ...
          resFitnessActivities[i].exercises.forEach((resExercise, ii) => {
            let athleteExercise = athleteFitnessActivities[i].exercises[ii];
            expect(resExercise.exercisesEquipmentId).to.eql(
            ...
            resExercise.exerciseSets.forEach((resExerciseSet, iii) => {
              let athleteExerciseSet =
                athleteFitnessActivities[i].exercises[ii].exerciseSets[iii];
    ...
    describe(":: GET /activities/team/:teamId/activity/fitness/member/:userTeamId/exercises/:userTeamActivityId ::", () => {
      it("Should return 404 when unknown teamId, userTeamId and/or userTeamActivityId", async () => {
        let res1 = await chai.requester
      ...
      it("Should return team member with assigned exercises of fitness activity", async () => {
        let res = await chai.requester
    ...
    describe(":: POST /activities/team/:teamId/activity/fitness/:activityId ::", () => {
      ...
      it("Should return 400 when unknown exercisesEquipmendId provided", async () => {
        let res = await chai.requester
          .post("/activities/team/1/activity/fitness/3")
          .send({
      ...
      it("Should return inserted exercise sets", async () => {
        let res = await chai.requester
          .post("/activities/team/1/activity/fitness/2")
          .send({
            data: [
    ...
    describe(":: PUT /activities/team/:teamId/activity/fitness/:activityId ::", () => {
      ...
      describe(":: Delete exercise set(s)", () => {
        it("Should return 404 when unknown id provided for deletion", async () => {
          let res = await chai.requester
            .put("/activities/team/1/activity/fitness/2")
            .send({
              data: [100],
        ...
        it("Should return 400 and not complete delete transaction if one of id(s) unknown", async () => {
          let res = await chai.requester
            .put("/activities/team/1/activity/fitness/2")
            .send({
              data: [3, 100],
    ...
    describe(":: PUT /activities/team/:teamId/activity/:activityId/fitness/member/:userTeamId/exercises ::", () => {
      it("Should return 403 when trying to update exerciseSet before the assigned day and activityStart", async () => {
        /**
      ...
      it("Should return status 200 when athlete updates all assigned exercise sets as done", async () => {
        let res = await chai.requester
      ...
      it("Should return updated information when other than athlete updates completed exercises", async () => {
        let res = await chai.requester
